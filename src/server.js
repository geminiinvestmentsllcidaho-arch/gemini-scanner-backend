...
app.post('/customer/paper-order/prepare', requireCustomerSession, requireCustomerSameOrigin, async (req, res) => {
  try {
    ...
    const localMockControl = process.env.CUSTOMER_PAPER_LOCAL_MOCK_EXERCISE_ENABLED === '1'
      ...
      : '';
    const manualTicket = `<section><h2>Manual Alpaca PAPER submission ticket</h2><p>Use these exact values in Alpaca PAPER. GeminiScanner will not transmit this order.</p><dl><dt>Symbol</dt><dd><strong>${handoff.order.symbol}</strong></dd><dt>Side</dt><dd><strong>${handoff.order.side.toUpperCase()}</strong></dd><dt>Quantity</dt><dd><strong>${handoff.order.qty}</strong></dd><dt>Order type</dt><dd><strong>${handoff.order.type}</strong></dd><dt>Time in force</dt><dd><strong>${handoff.order.timeInForce}</strong></dd><dt>Client order ID</dt><dd><strong>${handoff.order.clientOrderId}</strong></dd></dl><p><strong>Final action:</strong> open your Alpaca PAPER account, enter the ticket above, verify PAPER mode and the symbol/side/quantity, then press Alpaca's submit control yourself.</p><p>After manual submission, return to GeminiScanner for read-only reconciliation. Do not create a second order if the broker result is uncertain.</p></section>`;
    return res.status(200).type('html').send(`<!doctype html><html><body><main><h1>PAPER ${saved.mode} lifecycle ready</h1><p><strong>${saved.symbol}</strong> · quantity ${saved.quantity}</p><p>Preparation ID: ${saved.preparationId}</p><p>Lifecycle ID: ${handoff.lifecycleId}</p><p>Client order ID: ${handoff.order.clientOrderId}</p><p>Status: ${handoff.status}</p><p>GeminiScanner created or resolved the exact PAPER lifecycle and deterministic order handoff. Final broker submission remains blocked here; no broker contact, order placement, or account mutation occurred.</p>${manualTicket}${localMockControl}<p><a href="/customer/scanner/under-five">Back to scanner</a> · <a href="/customer/portfolio">Back to portfolio</a></p></main></body></html>`);
  } catch (error) {
