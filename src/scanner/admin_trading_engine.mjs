...
export function collectAdminTradingEngine({ runsDir = "runs", alpacaAccess = null, systemHealth = null, automaticPaper = null } = {}) {
  ...
  return Object.freeze({
    ...
    automaticPaper: Object.freeze({
      ...
      entryValidation: automaticPaper?.entryValidation ?? null,
...
<section class="p"><h2>Automatic Entry Validation</h2><div class="g"><div class="m">Status <strong>${esc(x.automaticPaper?.entryValidation?.status ?? "Unavailable")}</strong></div><div class="m">Last candidate <strong>${esc(x.automaticPaper?.entryValidation?.lastCandidate?.symbol ?? "Unavailable")}</strong><br>${esc(x.automaticPaper?.entryValidation?.lastCandidate?.decision ?? "Unavailable")}</div><div class="m">Blocker <strong>${esc(x.automaticPaper?.entryValidation?.lastCandidate?.blocker ?? "None")}</strong></div><div class="m">Allocation tier <strong>${esc(x.automaticPaper?.entryValidation?.allocationPercent ?? "Unavailable")}%</strong></div><div class="m">Proposed / executed qty <strong>${esc(x.automaticPaper?.entryValidation?.proposedQuantity ?? "Unavailable")} / ${esc(x.automaticPaper?.entryValidation?.executedQuantity ?? "Unavailable")}</strong></div><div class="m">Last automatic PAPER entry <strong>${esc(x.automaticPaper?.entryValidation?.lastEntry?.symbol ?? "Unavailable")}</strong><br>${esc(x.automaticPaper?.entryValidation?.lastEntry?.brokerOrderId ?? "Unavailable")}</div><div class="m">Fill / reconciliation <strong>${esc(x.automaticPaper?.entryValidation?.lastEntry?.filledQuantity ?? "Unavailable")} @ ${esc(x.automaticPaper?.entryValidation?.lastEntry?.averageFillPrice ?? "Unavailable")}</strong><br>${esc(x.automaticPaper?.entryValidation?.lastEntry?.reconciliationStatus ?? "Unavailable")}</div><div class="m">Correlation ID <code>${esc(x.automaticPaper?.entryValidation?.correlationId ?? "Unavailable")}</code></div></div><p>Read-only automatic-entry evidence. No controls, broker requests, order actions, strategy changes, threshold changes, sizing changes, AI-authority changes, or live-trading changes are available here.</p></section>
