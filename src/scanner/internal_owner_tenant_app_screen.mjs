...
export function buildInternalOwnerTenantAppScreen(options = {}) {
  ...
  return {
    ...
    bootstrapNotice: "Bootstrap profile protected by the existing operator token. Owner-specific authorization and tenant isolation are not implemented yet.",
    routes: {
      appHomeHref: "/app",
...
<section class="card"><h2>Security Foundation</h2><p><b>Authentication implemented:</b> ${esc(yesNo(screen.access.authenticationImplemented))}</p><p><b>Authentication mode:</b> ${esc(screen.access.authenticationMode)}</p><p><b>Route protection implemented:</b> ${esc(yesNo(screen.access.routeProtectionImplemented))}</p><p><b>Authorization enforced:</b> ${esc(yesNo(screen.access.authorizationEnforced))}</p><p><b>Tenant isolation implemented:</b> ${esc(yesNo(screen.access.tenantIsolationImplemented))}</p><p><b>Raw secrets exposed:</b> ${esc(yesNo(screen.credentials.rawSecretsExposed))}</p></section>
