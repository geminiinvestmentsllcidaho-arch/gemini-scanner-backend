...
export function buildInternalOwnerTenantAppScreen(options = {}) {
  ...
  return {
    ...
    bootstrapNotice: "Bootstrap profile protected by the existing operator token. Owner authorization and tenant-scoped customer path partitioning are implemented as a read-only foundation. Existing customer stores are not migrated and runtime store cutover remains disabled.",
    routes: {
      appHomeHref: "/app",
...
<section class="card"><h2>Security Foundation</h2><p><b>Authentication implemented:</b> ${esc(yesNo(screen.access.authenticationImplemented))}</p><p><b>Authentication mode:</b> ${esc(screen.access.authenticationMode)}</p><p><b>Route protection implemented:</b> ${esc(yesNo(screen.access.routeProtectionImplemented))}</p><p><b>Authorization enforced:</b> ${esc(yesNo(screen.access.authorizationEnforced))}</p><p><b>Authorization policy:</b> ${esc(screen.access.authorizationPolicy)}</p><p><b>Tenant isolation implemented:</b> ${esc(yesNo(screen.access.tenantIsolationImplemented))}</p><p><b>Tenant isolation policy:</b> ${esc(screen.access.tenantIsolationPolicy)}</p><p><b>Multi-tenant data partitioning implemented:</b> ${esc(yesNo(screen.access.multiTenantDataPartitioningImplemented))}</p><p><b>Data partitioning policy:</b> ${esc(screen.access.multiTenantDataPartitioningPolicy)}</p><p><b>Existing customer stores migrated:</b> ${esc(yesNo(screen.access.existingCustomerStoresMigrated))}</p><p><b>Runtime store cutover enabled:</b> ${esc(yesNo(screen.access.customerRuntimeStoreCutoverEnabled))}</p><p><b>Encrypted per-tenant credential storage:</b> ${esc(yesNo(screen.credentials.encryptedPerTenantStorageImplemented))}</p><p><b>Credential encryption policy:</b> ${esc(screen.credentials.encryptionPolicy)}</p><p><b>Credential key configured:</b> ${esc(yesNo(screen.credentials.keyConfigured))}</p><p><b>Credential store exists:</b> ${esc(yesNo(screen.credentials.storeExists))}</p><p><b>Credential store file:</b> ${esc(screen.credentials.storePathLabel)}</p><p><b>Migration required:</b> ${esc(yesNo(screen.credentials.migrationRequired))}</p><p><b>Raw secrets exposed:</b> ${esc(yesNo(screen.credentials.rawSecretsExposed))}</p></section>
