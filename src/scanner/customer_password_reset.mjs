...
export const VERSION = "customer_password_reset_v1";
...
export function createCustomerPasswordReset(account = {}, options = {}) {
  ...
  if (!clean(account.id) || !clean(account.email)) {
    throw new Error("password_reset_account_required");
  }
...
export function verifyCustomerPasswordResetToken(token, record = {}, options = {}) {
  const nowMs = Number.isFinite(options.nowMs) ? options.nowMs : Date.now();
