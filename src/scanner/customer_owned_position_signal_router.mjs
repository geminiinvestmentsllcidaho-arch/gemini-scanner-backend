...
export function routeCustomerOwnedPositionSignals(candidates=[],paperAccount={}){
 ...
 const scannerCandidates=[],exitAlerts=[],scaleInReviews=[],monitoredOwned=[];
 for(const candidate of list(candidates)){
  ...
  if(state(candidate)==="ENTER"){scaleInReviews.push({...base,alertType:"OWNED_POSITION_SCALE_IN_REVIEW",priority:"review",scaleInReady:false,separateApprovalRequired:true,blockedReasons:["SEPARATE_SCALE_IN_APPROVAL_REQUIRED"]});continue;}
  monitoredOwned.push({...base,alertType:"OWNED_POSITION_MONITOR"});
 }
 return Object.freeze({version:VERSION,scannerCandidates:Object.freeze(scannerCandidates),exitAlerts:Object.freeze(exitAlerts.sort(sort)),scaleInReviews:Object.freeze(scaleInReviews.sort(sort)),monitoredOwned:Object.freeze(monitoredOwned.sort(sort)),safety:Object.freeze({readOnly:true,paperOnly:true,decisionAssistOnly:true,automaticExitAllowed:false,automaticScaleInAllowed:false,brokerContactAllowed:false,orderPlacementAllowed:false,accountMutationAllowed:false})});
}
