...
<article class="decision-card state-${esc(card.stateClass)}">
  ...
  ${card.paperEnterExitGate?.exit?.visible ? `<section class="paper-control-preview exit-control-preview"><b>EXIT control preview</b><p class="paper-control priority-red">${esc(card.paperEnterExitGate.exit.label)}</p><p>Quantity: ${esc(card.paperEnterExitGate.exit.quantityPreview)}</p><p>${card.paperEnterExitGate.exit.ready ? "All preview gates passed." : `Blocked: ${esc(list(card.paperEnterExitGate.exit.blockedReasons).map(paperGateReasonLabel).join(" ") || "Unavailable")}`}</p><p>No broker contact or order placement.</p></section>` : ""}
  ${card.paperEnterExitGate?.enter?.visible ? `<section class="paper-control-preview enter-control-preview ${card.paperEnterExitGate.enter.ready ? "enter-ready" : "enter-blocked"}"><b>ENTER control preview — First controlled paper test</b><p class="paper-control ${card.paperEnterExitGate.enter.ready ? "bright-green" : "disabled-gray"}" aria-disabled="${card.paperEnterExitGate.enter.ready ? "false" : "true"}">${esc(card.paperEnterExitGate.enter.ready ? "ENTER / BUY — READY FOR 1-SHARE PAPER TEST" : "PAPER TEST BLOCKED")}</p><div class="decision-grid"><p><b>Normal suggested quantity</b><span>${esc(card.paperEnterExitGate.enter.suggestedQuantity ?? card.allocationPreview?.preview?.estimatedWholeShares ?? 0)}</span></p><p><b>Temporary test quantity</b><span>${esc(card.paperEnterExitGate.enter.firstTestQuantity ?? card.paperEnterExitGate.enter.quantityPreview ?? 0)} share</span></p><p><b>Estimated test cost</b><span>$${esc(card.paperEnterExitGate.enter.firstTestEstimatedCost ?? 0)}</span></p></div><p>${card.paperEnterExitGate.enter.ready ? "All preview gates passed. The first controlled test remains limited to one paper share." : `Blocked: ${esc(list(card.paperEnterExitGate.enter.blockedReasons).map(paperGateReasonLabel).join(" ") || "Unavailable")}`}</p>${card.paperEnterExitGate.enter.ready ? `<form method="post" action="/customer/paper-order/prepare"><input type="hidden" name="mode" value="ENTER"><input type="hidden" name="symbol" value="${esc(card.symbol)}"><input type="hidden" name="quantity" value="1"><input type="hidden" name="paperOnly" value="true"><button type="submit" class="paper-control bright-green">Prepare 1-share PAPER ENTER</button></form>` : ""}<p>Preparation only. No broker contact or order placement.</p></section>` : ""}
