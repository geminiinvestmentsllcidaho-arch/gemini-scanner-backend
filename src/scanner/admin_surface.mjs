...
<div class="card"><h3>Automatic Alpaca PAPER Execution</h3><p>Read-only runtime status for continuity, ENTER, SCALE, and EXIT.</p><p><strong>ENTER:</strong> ${surface.tradingEngine?.automaticPaper?.enter?.enabled ? "ARMED" : "OFF"} · <strong>SCALE:</strong> ${surface.tradingEngine?.automaticPaper?.scale?.enabled && surface.tradingEngine?.automaticPaper?.scale?.scaleInEnabled && surface.tradingEngine?.automaticPaper?.scale?.scaleOutEnabled ? "ARMED" : "OFF"} · <strong>EXIT:</strong> ${surface.tradingEngine?.automaticPaper?.exit?.enabled && surface.tradingEngine?.automaticPaper?.exit?.running ? "ARMED" : "OFF"}</p><p><strong>Readiness:</strong> ${esc(surface.tradingEngine?.automaticPaper?.readiness?.status ?? "Unavailable")} · <strong>Assurance:</strong> ${surface.tradingEngine?.automaticPaper?.assurance?.report?.healthy === true ? "HEALTHY" : surface.tradingEngine?.automaticPaper?.assurance?.report?.healthy === false ? "UNHEALTHY" : "Unavailable"} · <strong>Safe repair:</strong> ${surface.tradingEngine?.automaticPaper?.assurance?.safeRepairAllowed === true ? "AUTHORIZED" : "OFF"}</p><p><strong>Lifecycle:</strong> ${esc(surface.tradingEngine?.automaticPaper?.lifecycle?.state ?? "Unavailable")} · <strong>Symbol:</strong> ${esc(surface.tradingEngine?.automaticPaper?.lifecycle?.selectedSymbol ?? "Unavailable")}</p><a class="admin-action" href="/admin/trading-engine">Open trading engine</a></div>
