...
export const VERSION = 'paper_auto_execution_run_once_authorization_v1'
export const REQUIRED_PHRASE = 'I_APPROVE_ONE_DISABLED_PAPER_AUTO_RUN_ONCE'
...
function readLatch(file) {
  ...
  try {
    ...
    if (error?.code === 'ENOENT') return { consumed: false, blocker: null, record: null }
    return { consumed: true, blocker: 'authorization_latch_unreadable', record: null }
  }
}

export function evaluatePaperAutoRunOnceAuthorization(input = {}, nowMs = Date.now()) {
  const blockers = []
  ...
  if (clean(input.env?.PAPER_AUTO_RUN_ONCE_AUTHORIZATION_ENABLED) !== '1') blockers.push('authorization_disabled_by_env')
  ...
  if (phrase !== REQUIRED_PHRASE) blockers.push('exact_authorization_phrase_required')
  if (scope !== 'paper_auto_run_once_only') blockers.push('exact_authorization_scope_required')
  ...
  return Object.freeze({
    ...
    version: VERSION,
    status: blockers.length === 0 ? 'AUTHORIZED_FOR_ONE_DISABLED_RUN_ONCE' : 'BLOCKED',
...
export function consumePaperAutoRunOnceAuthorization(input = {}, nowMs = Date.now()) {
  const result = evaluatePaperAutoRunOnceAuthorization(input, nowMs)
...
export default { VERSION, REQUIRED_PHRASE, evaluatePaperAutoRunOnceAuthorization, consumePaperAutoRunOnceAuthorization }
