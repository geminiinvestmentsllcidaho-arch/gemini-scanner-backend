...
<section class="card performance-${esc(dashboard.performanceReport?.tone ?? "neutral")}"><b>Total earnings — ${esc(dashboard.performanceReport?.period ?? "daily")}</b><nav class="performance-periods" aria-label="Performance period">${["daily","weekly","monthly","yearly","ytd","lifetime"].map((period) => `<a class="${dashboard.performanceReport?.period === period ? "active" : ""}" href="${esc(dashboard.route ?? "/customer-zero/under-five-scanner")}?period=${period}">${period.toUpperCase()}</a>`).join("")}</nav><p>Realized: $${esc(dashboard.performanceReport?.realizedPl ?? 0)} | Unrealized: $${esc(dashboard.performanceReport?.unrealizedPl ?? 0)} | Combined: $${esc(dashboard.performanceReport?.totalPl ?? 0)} | Net after costs: $${esc(dashboard.performanceReport?.netAfterCosts ?? 0)}</p><p>Winners: ${esc(dashboard.performanceReport?.winners ?? 0)} | Losers: ${esc(dashboard.performanceReport?.losers ?? 0)} | Win rate: ${esc(dashboard.performanceReport?.winRatePct ?? 0)}%</p><p>Average gain: $${esc(dashboard.performanceReport?.averageGain ?? 0)} | Average loss: $${esc(dashboard.performanceReport?.averageLoss ?? 0)} | Largest gain: $${esc(dashboard.performanceReport?.largestGain ?? 0)} | Largest loss: $${esc(dashboard.performanceReport?.largestLoss ?? 0)}</p><p>Fees: $${esc(dashboard.performanceReport?.fees ?? 0)} | Slippage: $${esc(dashboard.performanceReport?.slippage ?? 0)}</p><p>Starting equity: $${esc(dashboard.performanceReport?.startingEquity ?? 0)} | Ending equity: $${esc(dashboard.performanceReport?.endingEquity ?? 0)} | Peak equity: $${esc(dashboard.performanceReport?.peakEquity ?? 0)} | Drawdown: $${esc(dashboard.performanceReport?.drawdown ?? 0)} (${esc(dashboard.performanceReport?.drawdownPct ?? 0)}%)</p><p>Period snapshots: ${esc(dashboard.performanceReport?.periodRecordCount ?? 0)} | Period start: ${esc(dashboard.performanceReport?.periodStartTs ?? "Unavailable")} | Period end: ${esc(dashboard.performanceReport?.periodEndTs ?? "Unavailable")}</p><p>Data timestamp: ${esc(dashboard.performanceReport?.sourceTs ?? "Unavailable")} | Status: ${dashboard.performanceReport?.stale === true ? "STALE — READ ONLY" : "Current — read only"}</p></section>
