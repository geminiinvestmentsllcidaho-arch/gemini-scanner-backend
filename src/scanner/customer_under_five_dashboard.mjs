...
import { routeCustomerOwnedPositionSignals } from "./customer_owned_position_signal_router.mjs";
...
export function buildCustomerUnderFiveDashboard(source = {}, options = {}) {
  ...
  const candidates = card.candidates.map((candidate, index) => {
    ...
    return {
      ...
      allocationPreview: buildCustomerZeroReadonlyAllocationPreview(sourceCandidate, {
        paperEquity: options.equity ?? options.paperAccount?.account?.equity,
  ...
  const prioritizedCandidates = [...ownedPositionSignals.scannerCandidates].sort((left, right) => {
    ...
    const confidenceDelta=Number(right?.readonlyPotentialScore??-1)-Number(left?.readonlyPotentialScore??-1);
...
export function renderCustomerUnderFiveDashboardHtml(dashboard = {}, account = null) {
  ...
  const scaleOutReviews=Array.isArray(dashboard?.scaleOutReviews)?dashboard.scaleOutReviews:[];
  ...
  const exitAlertsHtml=positionAlerts.length?`<section class="owned-exit-stack" data-owned-exit-alerts data-alert-count="${esc(positionAlerts.length)}"><div class="owned-exit-banner" role="alert"><b>URGENT PAPER POSITION EXIT REVIEW</b><button type="button" data-enable-exit-alerts>Enable EXIT sound and notifications</button></div>${positionAlerts.map(a=>`<article><h2>${esc(a.symbol)} — EXIT REVIEW</h2><p>Owned quantity: ${esc(a.ownedPositionQty??"Unavailable")} · Position return: ${esc(Number.isFinite(Number(a.ownedReturnPct))?`${Number(a.ownedReturnPct).toFixed(2)}%`:"Unavailable")} · Score: ${esc(a.readonlyPotentialScore??"Unavailable")}</p><p><b>Reason:</b> ${esc(a.ownedExitReviewReason==="OWNED_POSITION_HARD_LOSS_REVIEW"?"Position loss reached the hard-loss review threshold.":a.ownedExitReviewReason==="OWNED_POSITION_EARLY_LOSS_REVIEW"?"Owned-position loss reached the early-loss review threshold.":"Owned-position risk conditions require manual review.")}</p><p>Manual review only. No automatic exit, broker order, or account mutation.</p></article>`).join("")}</section>`:"";
  const scaleOutHtml=scaleOutReviews.length?`<section class="card owned-scale-out" data-owned-scale-out-reviews><b>Partial profit-protection reviews for owned positions</b><p>These profitable positions are showing fresh signs of weakening. They remain separate from full EXIT alerts and new opportunities.</p>${scaleOutReviews.map(a=>`<article><h3>${esc(a.symbol)} — REVIEW A PARTIAL SALE</h3><p>Owned quantity: ${esc(a.ownedPositionQty??"Unavailable")} · Current return: ${Number.isFinite(Number(a.ownedReturnPct))?`${esc(Number(a.ownedReturnPct).toFixed(2))}%`:"Unavailable"} · Strength score: ${esc(a.readonlyPotentialScore??"Unavailable")} · Suggested review quantity: ${esc(a.ownedScaleOutSuggestedQty??"Unavailable")}</p><p>Fresh evidence shows weakening momentum while the position remains profitable. This is a partial profit-protection review, not a full EXIT alert.</p><p>This scanner view does not submit the sale. Any PAPER execution is handled by the separate PAPER execution path under its runtime safety gates.</p></article>`).join("")}</section>`:"";
  const scaleInHtml=scaleInReviews.length?`<section class="card owned-scale-in" data-owned-scale-in-reviews><b>Add-on reviews for owned positions</b><p>These are positions you already own, so they remain separate from new opportunities.</p>${scaleInReviews.map(a=>`<article><h3>${esc(a.symbol)} — REVIEW AN ADD-ON</h3><p>Owned quantity: ${esc(a.ownedPositionQty??"Unavailable")} · Average entry: ${esc(a.ownedAverageEntryPrice??"Unavailable")} · Current return: ${Number.isFinite(Number(a.ownedReturnPct))?`${esc(Number(a.ownedReturnPct).toFixed(2))}%`:"Unavailable"} · Strength score: ${esc(a.readonlyPotentialScore??"Unavailable")}</p><p>The scanner found fresh, confirmed strength while the position remains profitable. This is a review only, not permission to buy more.</p><p>This scanner view does not submit an add-on order. Any PAPER execution is handled by the separate PAPER execution path under its runtime safety gates. The scanner will not recommend averaging down on a losing position.</p></article>`).join("")}</section>`:"";
  const monitoredOwnedHtml=monitoredOwned.length?`<section class="card owned-position-monitor" data-owned-position-monitors data-monitor-count="${esc(monitoredOwned.length)}"><b>Owned paper positions under active review</b><p>These positions are monitored independently from the opportunity price range.</p>${monitoredOwned.map(a=>`<article><h3>${esc(a.symbol)} — MONITOR</h3><p>Owned quantity: ${esc(a.ownedPositionQty??"Unavailable")} · Average entry: ${esc(a.ownedAverageEntryPrice??"Unavailable")} · Current price: ${esc(a.currentPrice??a.price??"Unavailable")} · Position return: ${esc(Number.isFinite(Number(a.ownedReturnPct))?`${Number(a.ownedReturnPct).toFixed(2)}%`:"Unavailable")}</p><p><b>Current assessment:</b> ${esc(String(a.resultState??a.decision??"WATCH").replaceAll("_"," "))}. No EXIT review is active.</p><p>Read-only monitoring. No automatic exit, scale-in, or order placement.</p></article>`).join("")}</section>`:"";

  return `<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>${esc(dashboard.title)}</title>
${renderGlobalThemeCss({ surface: "customer" })}
<style>
.wrap{padding:42px 14px 72px}.hero,.card,.decision-card{background:rgba(0,0,0,.72)!important;color:var(--gs-text)!important;border:1px solid var(--gs-line)}.wrap{max-width:820px;margin:auto}.hero,.card,.decision-card{background:#fff;border-radius:18px;padding:15px;margin:10px 0;box-shadow:0 8px 22px #0001}.hero{background:#111;color:#fff}.performance-positive{border-left:8px solid #159447}.performance-negative{border-left:8px solid #c62020}.performance-neutral{border-left:8px solid #737983}.performance-periods{display:flex;flex-wrap:wrap;gap:7px;margin:10px 0}.performance-periods a{padding:8px 10px;border-radius:999px;background:#eceff2;color:#111;text-decoration:none;font-weight:800}.performance-periods a.active{background:#111;color:#fff}.hero h1{margin:.2rem 0}.decision-card{border-left:6px solid #8a8f98;padding:12px}.decision-card-head{display:flex;align-items:flex-start;justify-content:space-between;gap:10px}.decision-card h2{margin:0;font-size:1.45rem}.company-name{margin:.12rem 0;color:#c8d2d8;font-size:.88rem;font-weight:650}.state-badge{border-radius:999px;padding:7px 10px;font-size:.78rem;font-weight:900;white-space:nowrap;background:#eceff2}.decision-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:6px;margin-top:9px}.decision-grid p{margin:0;padding:8px;background:rgba(7,20,25,.94);border:1px solid rgba(24,215,255,.3);border-radius:10px;color:#f4fbff;min-width:0}.decision-grid b,.decision-grid span{display:block}.decision-grid b{font-size:.7rem;color:#9fdde8;text-transform:uppercase;letter-spacing:.03em}.decision-grid span{margin-top:3px;font-size:.9rem;font-weight:800;overflow-wrap:anywhere}.timestamp{margin:8px 0;font-size:.78rem;color:#c4d0d6}.reasons{font-size:.88rem}.reasons ul{margin:.25rem 0 .55rem;padding-left:1.1rem}.reasons li{margin:.16rem 0}.allocation-preview{margin:8px 0;padding:9px;border:1px solid rgba(57,255,32,.28);border-radius:12px;background:rgba(3,17,14,.78)}.allocation-preview>.decision-grid{margin-top:7px}.detail-link{display:block;text-align:center;padding:9px;border-radius:10px;background:rgba(24,215,255,.12);border:1px solid rgba(24,215,255,.5);color:#dffaff;text-decoration:none;font-weight:850}.state-enter{border-left-color:#159447}.state-enter .state-badge{background:#dff7e7;color:#11652e}.state-exit{border-left-color:#ff1f1f;box-shadow:0 0 18px rgba(255,31,31,.38)}.state-exit .state-badge{background:#ff1111;color:#fff;border:1px solid #ffb3b3;box-shadow:0 0 16px rgba(255,17,17,.78);animation:gs-exit-flash .8s steps(2,end) infinite}.state-wait,.state-watch{border-left-color:#d39b00}.state-wait .state-badge,.state-watch .state-badge{background:#fff2c8;color:#765800}.state-do-not-enter,.state-blocked,.state-stale-data{border-left-color:#c62020}.state-do-not-enter .state-badge,.state-blocked .state-badge,.state-stale-data .state-badge{background:#ffe0e0;color:#8a1111}.state-no-setup{border-left-color:#737983}.scan-status-bar{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:10px 0}.scan-status-item{padding:12px 14px;border-radius:14px;border:1px solid var(--gs-line);background:rgba(0,0,0,.82);text-align:center;font-weight:950;letter-spacing:.04em}.market-open{color:#7dff9b;border-color:#18a84a;box-shadow:0 0 16px rgba(24,168,74,.28)}.market-closed{color:#ff2929;border-color:#ff1f1f;text-shadow:0 0 8px rgba(255,31,31,.95);box-shadow:0 0 22px rgba(255,31,31,.58)}.scan-countdown{color:#7be9ff;border-color:#18d7ff;box-shadow:0 0 16px rgba(24,215,255,.22)}.paper-control-preview{margin:12px 0;padding:12px;border-radius:14px;background:#f6f7f8}.paper-control{display:block;text-align:center;padding:13px;border-radius:12px;font-weight:950}.bright-green{background:#18a84a;color:#fff}.disabled-gray{background:#4f575d;color:#e9eef1;border:1px solid #7f898f;box-shadow:none}.enter-blocked{border:2px solid #7f898f!important;opacity:.92}.enter-ready{border:2px solid #159447!important}.priority-red{background:#ff1111;color:#fff;border:1px solid #ffb3b3;box-shadow:0 0 18px rgba(255,17,17,.78);animation:gs-exit-flash .8s steps(2,end) infinite}.exit-control-preview{border:2px solid #ff1f1f;box-shadow:0 0 18px rgba(255,31,31,.42)}@keyframes gs-exit-flash{0%,48%{opacity:1;filter:brightness(1.35)}49%,100%{opacity:.35;filter:brightness(.72)}}@media(prefers-reduced-motion:reduce){.state-exit .state-badge,.priority-red{animation:none}}.owned-exit-stack{margin:12px}.owned-exit-banner,.owned-exit-stack article{padding:14px;margin:8px 0;border:3px solid #ff1f1f;border-radius:14px;background:#310000;color:#fff;box-shadow:0 0 24px #ff1f1f;animation:gs-exit-flash .8s steps(2,end) infinite}.owned-exit-banner{display:flex;gap:10px;flex-direction:column}.owned-exit-banner button{padding:10px;font-weight:900}.owned-scale-out{border:2px solid #18d7ff!important}.owned-scale-out article{padding:10px;border:1px solid #18d7ff;border-radius:10px;margin-top:8px}.owned-scale-in{border:2px solid #d39b00!important}.owned-scale-in article{padding:10px;border:1px solid #d39b00;border-radius:10px;margin-top:8px}.enter-control-preview{border:2px solid #159447}@media(max-width:700px){.wrap{padding:24px 12px 52px}.decision-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.scan-status-bar{grid-template-columns:1fr}.hero,.card,.decision-card{min-width:0}.decision-card-head{flex-wrap:wrap}.decision-card-head>*{min-width:0}.state-badge{max-width:100%;white-space:normal;text-align:center}.performance-periods a{flex:1 1 auto;text-align:center}}@media(max-width:420px){.wrap{padding-left:10px;padding-right:10px}.decision-card{padding:10px}.decision-grid{grid-template-columns:minmax(0,1fr);gap:5px}.decision-grid p{padding:7px}.decision-card-head{align-items:flex-start}.state-badge{font-size:.72rem}.owned-exit-stack{margin:8px 0}}
...
${monitoredOwnedHtml}
${resultRows}
<section class="card"><b>Customer safety:</b> Decision assistance only. No order-placement, broker-contact, or account-modification controls are available.</section>
</main>
${renderGlobalFooter()}
${marketOpen ? '<script src="/assets/customer-scanner-countdown.js" defer></script>' : ""}
${positionAlerts.length ? '<script src="/assets/customer-owned-position-alerts.js" defer></script>' : ""}
</body></html>`;
}
