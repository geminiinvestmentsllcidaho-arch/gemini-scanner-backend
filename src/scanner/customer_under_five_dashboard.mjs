...
export function renderCustomerUnderFiveDashboardHtml(dashboard = {}, account = null) {
  ...
  const scaleOutHtml=scaleOutReviews.length?`<section class="card owned-scale-out" data-owned-scale-out-reviews><b>Owned-position profit-protection reviews</b><p>These are profitable positions showing fresh weakening evidence. They are kept separate from full EXIT alerts and new opportunities.</p>${scaleOutReviews.map(a=>`<article><h3>${esc(a.symbol)} — REVIEW SELLING PART</h3><p>Owned quantity: ${esc(a.ownedPositionQty??"Unavailable")} | Current return: ${Number.isFinite(Number(a.ownedReturnPct))?`${esc(Number(a.ownedReturnPct).toFixed(2))}%`:"Unavailable"} | Strength score: ${esc(a.readonlyPotentialScore??"Unavailable")} | Suggested review quantity: ${esc(a.ownedScaleOutSuggestedQty??"Unavailable")}</p><p>Fresh evidence shows weakening momentum while the position remains profitable. This is a partial profit-protection review, not a full EXIT signal.</p><p>No automatic sale, broker order, or account mutation. Separate approval and quantity review remain required.</p></article>`).join("")}</section>`:"";
  const scaleInHtml=scaleInReviews.length?`<section class="card owned-scale-in" data-owned-scale-in-reviews><b>Owned-position add-on reviews</b><p>These are positions you already own. They are kept separate from new opportunities.</p>${scaleInReviews.map(a=>`<article><h3>${esc(a.symbol)} — REVIEW ADDING MORE</h3><p>Owned quantity: ${esc(a.ownedPositionQty??"Unavailable")} | Average entry: ${esc(a.ownedAverageEntryPrice??"Unavailable")} | Current return: ${Number.isFinite(Number(a.ownedReturnPct))?`${esc(Number(a.ownedReturnPct).toFixed(2))}%`:"Unavailable"} | Strength score: ${esc(a.readonlyPotentialScore??"Unavailable")}</p><p>The scanner found fresh, profitable, confirmed strength. This is a review only—not permission to buy more.</p><p>Adding more remains blocked until a separate approval and position-size review are completed. The scanner will not recommend averaging down on a losing position.</p></article>`).join("")}</section>`:"";
...
<b>Result order:</b> Opportunity ENTER candidates appear first, ranked from strongest potential score to weakest. Equal scores are sorted by symbol. Owned symbols are monitored separately: EXIT signals become urgent alerts and repeat ENTER signals become scale-in reviews.
</section>
