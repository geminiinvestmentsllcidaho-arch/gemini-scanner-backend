...
export function buildAiLogicOneShotNonruntimeInvocationContract({operatorApproval:a,consumptionStoreRecord:c,decisionEvidence:d,orchestratorContract:o,boundaryEvidence:b,targetPath,expectedPreimageHash,operationId}={}){
 ...
 if(b?.version!=="ai_logic_execution_boundary_gate_v1"||b?.eligible!==true||b?.applyEligibilityOnly!==true||b?.paperOnly!==true)r.push("BOUNDARY_INVALID");
 ...
 if(id.action==="ROLLBACK"&&(!P(d?.promotionDecisionRecordId)||d?.acceptanceRecordId!==a?.acceptanceRecordId))r.push("ROLLBACK_DECISION_BINDING_INVALID");
