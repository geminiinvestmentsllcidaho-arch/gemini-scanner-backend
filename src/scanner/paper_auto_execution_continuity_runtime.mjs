...
export function createPaperAutoExecutionContinuityRuntime(o={}){
 const {env=process.env,getActiveLifecycleFile,setActiveLifecycleFile,getScanSnapshot,runsDir='runs',idFactory=()=>crypto.randomUUID(),storeFactory=f=>new PaperAutoExecutionLifecycleStore({filePath:f}),now=Date.now,appendEntryValidation=appendPaperAutoExecutionEntryValidationRecord,entryValidationEvidencePath=path.join(runsDir,'paper_auto_execution_entry_validation.jsonl')}=o
