...
export function createPaperAutoExecutionContinuityRuntime(o={}){
 ...
 let inFlight=null,cycles=0,lastStatus='NOT_RUN',lastLifecycleFile=null,lastLifecycle=null
 ...
 const cycle=async()=>{cycles++;if(!on(env,'PAPER_AUTO_CONTINUITY_ENABLED')){lastStatus='CONTINUITY_DISABLED_BY_ENV';return diagnostics()}
  const activeFile=clean(typeof getActiveLifecycleFile==='function'?await getActiveLifecycleFile():env.PAPER_AUTO_EXIT_MONITOR_LIFECYCLE_PATH);let active=null
  ...
  lastStatus='FRESH_CANDIDATE_LIFECYCLE_CREATED';lastLifecycleFile=file;lastLifecycle=Object.freeze({...life,enterIdentity});return diagnostics()}
