...
export const VERSION = 'paper_auto_execution_scale_order_lookup_v1'
const PAPER_BASE_URL = 'https://paper-api.alpaca.markets'
const clean = value => String(value ?? '').trim()

export async function fetchAlpacaPaperOrderByClientOrderIdReadonly({
  clientOrderId,
  env = process.env,
  ...
  if (!id) throw new Error('paper_scale_order_lookup_client_order_id_required')
  ...
  if (baseUrl !== PAPER_BASE_URL) throw new Error('paper_scale_order_lookup_paper_host_required')
  if (!apiKey || !apiSecret) throw new Error('paper_scale_order_lookup_credentials_required')
  if (typeof fetchImpl !== 'function') throw new Error('paper_scale_order_lookup_fetch_required')
  ...
  if (!response.ok || !body || typeof body !== 'object' || Array.isArray(body)) {
    throw new Error(`paper_scale_order_lookup_fetch_failed:${response.status}`)
  }
  if (clean(body.client_order_id) !== id) throw new Error('paper_scale_order_lookup_identity_mismatch')
...
export default { VERSION, fetchAlpacaPaperOrderByClientOrderIdReadonly }
