...
export function buildAiLogicExecutionIntentAcknowledgement({executionIntent:i}={}){
 ...
 const id={approvalRecordId:i?.approvalRecordId,nonce:i?.nonce,action:i?.action,decisionRecordId:i?.decisionRecordId,candidateSourceHash:i?.candidateSourceHash,currentSourceCommit:i?.currentSourceCommit,targetSourceCommit:i?.targetSourceCommit};
 if(!["PROMOTION","ROLLBACK"].includes(id.action)||![id.approvalRecordId,id.nonce,id.decisionRecordId,id.candidateSourceHash,id.currentSourceCommit,id.targetSourceCommit].every(P)||id.currentSourceCommit===id.targetSourceCommit)r.push("IDENTITY_INVALID");
