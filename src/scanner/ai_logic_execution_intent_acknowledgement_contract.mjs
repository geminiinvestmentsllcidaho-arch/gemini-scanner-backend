...
export function buildAiLogicExecutionIntentAcknowledgement({executionIntent:i}={}){
 ...
 const id={approvalRecordId:i?.approvalRecordId,nonce:i?.nonce,action:i?.action,decisionRecordId:i?.decisionRecordId,candidateSourceHash:i?.candidateSourceHash,candidatePath:i?.candidatePath,candidateTopic:i?.candidateTopic,currentSourceCommit:i?.currentSourceCommit,targetSourceCommit:i?.targetSourceCommit};
 if(!["PROMOTION","ROLLBACK"].includes(id.action)||![id.approvalRecordId,id.nonce,id.decisionRecordId,id.candidateSourceHash,id.candidatePath,id.candidateTopic,id.currentSourceCommit,id.targetSourceCommit].every(P)||id.currentSourceCommit===id.targetSourceCommit)r.push("IDENTITY_INVALID");
