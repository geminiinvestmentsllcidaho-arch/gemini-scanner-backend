    ...
    const response = await fetchImpl(config.endpoint, {
      ...
      body: JSON.stringify({
        ...
        instructions: [
          ...
          "Treat input.strategyAuthorizationPolicy as canonical read-only execution-policy context sourced from the deterministic strategy authorization module.",
          "The deterministic strategy authorization policy is the execution authority; AI must never authorize, override, loosen, bypass, mutate, or redefine it.",
          "Missing strategy authorization evidence is not authorization. Portfolio/root Stage-2 permission is not symbol-level authorization, and rank or normalizedScore are not authorization thresholds.",
          "Ranking targetPositionPct and ranking maxPositionPct are diagnostic ranking outputs and are not execution-sizing authority. AI must not redefine or override execution sizing.",
          ...
          "Use input.paperExecutionLifecycleEvidence only as bounded, sanitized Alpaca PAPER execution-lifecycle evidence for studying candidate selection, entry revalidation, monitoring, scale actions, exit reasoning, and terminal outcomes.",
          "Never infer missing lifecycle or scale fields, and never treat paperExecutionLifecycleEvidence as execution authority.",
          "Never treat paperExecutionLifecycleEvidence as permission to place orders, contact the broker, mutate accounts, learn automatically, patch code, mutate scanner logic, or change thresholds.",
