...
const KGLOCKS="persistenceAllowed,productionRuntimeWiringAllowed,strategySwitchingAllowed,brokerContactAllowed,orderPlacementAllowed,liveTradingAllowed,accountMutationAllowed,rollbackExecutable,promotionEligible".split(",");const DLOCKS="productionRuntimeWiringAllowed,persistenceAllowed,promotionAllowed,promotionExecutionAllowed,rollbackExecutionAllowed,brokerContactAllowed,orderPlacementAllowed,liveTradingAllowed,accountMutationAllowed,immutablePolicyMutationAllowed,thresholdMutationAllowed,sizingMutationAllowed,allocationMutationAllowed".split(",");const requireLocks=(o,reasons,label,locks=LOCKS)=>{for(const k of locks)if(o?.[k]!==false)reasons.push(`${label}_${k}_MUST_BE_FALSE`);};
export function validateAiLogicEvidenceSchemas({decisionEvidence:d,knownGood:k,executionPreview:x,operatorApproval:a,consumptionStoreRecord:c,action}={}){
 ...
 if(d?.localJsonlOnly!==true||d?.immutableManifestStatus!=="IMMUTABLE_MANIFEST_VERIFIED")reasons.push("DECISION_MODE_INVALID"); if(action==="ROLLBACK"&&(d?.rollbackTargetIdentified!==true||d?.rollbackDecisionEvidenceOnly!==true))reasons.push("ROLLBACK_EVIDENCE_INVALID"); if(action==="PROMOTION"&&!present(d?.acceptanceRecordId))reasons.push("DECISION_ACCEPTANCE_REQUIRED");
 requireFields(d,["recordId","candidateId","knownGoodRecordId","replayId","sourceCommitBefore","sourceCommitAfter","candidateSourceHash","candidatePath","candidateTopic"],reasons,"DECISION");
 if(k?.version!=="ai_logic_known_good_record_v1"||k?.valid!==true||k?.status!=="KNOWN_GOOD_RECORD_VALID"||k?.rollbackTargetIdentified!==true)reasons.push("KNOWN_GOOD_INVALID");requireLocks(k,reasons,"KNOWN_GOOD",KGLOCKS);
 if(x?.version!=="ai_logic_execution_preview_contract_v1"||x?.eligible!==true||x?.previewOnly!==true||x?.paperOnly!==true||x?.gitEffects!=="NONE")reasons.push("PREVIEW_INVALID");requireLocks(x,reasons,"PREVIEW");requireFields(x,["approvalRecordId","nonce","action","decisionRecordId","candidateSourceHash","candidatePath","candidateTopic","currentSourceCommit","targetSourceCommit"],reasons,"PREVIEW");
 if(a?.version!=="ai_logic_operator_approval_record_v1"||a?.valid!==true||a?.explicitlyApproved!==true||a?.oneShot!==true||a?.paperOnly!==true||a?.localJsonlOnly!==true||!present(a?.recordId))reasons.push("APPROVAL_INVALID");requireLocks(a,reasons,"APPROVAL");
 ...
 const ai=a??{};
 for(const q of ["decisionRecordId","candidateId","knownGoodRecordId","replayId","sourceCommitBefore","sourceCommitAfter","candidateSourceHash","candidatePath","candidateTopic","nonce"])if(!present(ai[q]))reasons.push("APPROVAL_IDENTITY_INCOMPLETE");
 if(action==="PROMOTION"&&!present(ai.acceptanceRecordId))reasons.push("APPROVAL_ACCEPTANCE_REQUIRED");
 for(const q of ["approvalRecordId","nonce","action","decisionRecordId","candidateSourceHash","candidatePath","candidateTopic","currentSourceCommit","targetSourceCommit"])if(x?.[q]!==c?.[q])reasons.push("PREVIEW_CONSUMPTION_BINDING_MISMATCH");
 ...
 for(const q of ["candidateSourceHash","candidatePath","candidateTopic"])if(x?.[q]!==ai[q]||c?.[q]!==ai[q])reasons.push("CANDIDATE_PROVENANCE_APPROVAL_BINDING_MISMATCH");
 ...
 for(const q of ["candidateId","knownGoodRecordId","replayId","sourceCommitBefore","sourceCommitAfter","candidateSourceHash","candidatePath","candidateTopic"])if(d?.[q]!==ai[q])reasons.push("DECISION_APPROVAL_BINDING_MISMATCH");
