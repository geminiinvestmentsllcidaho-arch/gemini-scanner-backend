...
export function buildAiLogicExecutionPlan({authorityGate:g,immutableManifest:m,operatorApproval:a,consumptionStoreRecord:c,now=new Date().toISOString()}={}){
 ...
 const id={approvalRecordId:g?.approvalRecordId,nonce:g?.nonce,action:g?.action,decisionRecordId:g?.decisionRecordId,currentSourceCommit:g?.currentSourceCommit,targetSourceCommit:g?.targetSourceCommit};
 if(!["PROMOTION","ROLLBACK"].includes(id.action)||![id.approvalRecordId,id.nonce,id.decisionRecordId,id.currentSourceCommit,id.targetSourceCommit].every(present)) reasons.push("IDENTITY_INVALID");
 const ai=a??{};
 if(a?.recordId!==id.approvalRecordId||ai.nonce!==id.nonce||ai.action!==id.action||ai.decisionRecordId!==id.decisionRecordId) reasons.push("APPROVAL_BINDING_MISMATCH");
