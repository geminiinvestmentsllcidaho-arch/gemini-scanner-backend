...
export function buildAiLogicExecutionPreviewContract({consumptionRecord,immutableManifest,decisionIdentity,knownGood,candidateTarget}={}){
  ...
  for(const k of ["approvalRecordId","nonce","action","decisionRecordId","candidateSourceHash","candidatePath","candidateTopic","currentSourceCommit","targetSourceCommit"]) if(!present(consumptionRecord?.[k])) reasons.push("CONSUMPTION_IDENTITY_INCOMPLETE");
  ...
  if(decisionIdentity?.candidatePath!==consumptionRecord?.candidatePath) reasons.push("DECISION_CANDIDATE_PATH_MISMATCH");
  if(decisionIdentity?.candidateTopic!==consumptionRecord?.candidateTopic) reasons.push("DECISION_CANDIDATE_TOPIC_MISMATCH");
  ...
  return Object.freeze({
    ...
    candidatePath:consumptionRecord?.candidatePath??null,
    candidateTopic:consumptionRecord?.candidateTopic??null,
