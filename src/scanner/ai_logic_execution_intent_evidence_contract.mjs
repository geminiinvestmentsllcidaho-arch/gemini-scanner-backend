...
export function buildAiLogicExecutionIntentEvidence({executionPlan:p}={}){
 ...
 const id={approvalRecordId:p?.approvalRecordId,nonce:p?.nonce,action:p?.action,decisionRecordId:p?.decisionRecordId,candidateSourceHash:p?.candidateSourceHash,candidatePath:p?.candidatePath,candidateTopic:p?.candidateTopic,currentSourceCommit:p?.currentSourceCommit,targetSourceCommit:p?.targetSourceCommit};
