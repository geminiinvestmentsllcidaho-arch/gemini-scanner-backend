...
<p class="portfolio-safety">Paper only · Read only · No broker contact · No order placement · No account mutation.</p>
  ...
  const performanceTotal = performance?.netAfterCosts ?? performance?.totalPl;
  const performanceTotalText = performanceTotal == null ? "Unavailable" : `$${esc(performanceTotal)}`;
  const performanceRealizedText = performance?.realizedPl == null ? "Unavailable" : `$${esc(performance.realizedPl)}`;
  const performanceUnrealizedText = performance?.unrealizedPl == null ? "Unavailable" : `$${esc(performance.unrealizedPl)}`;
  const performanceCombinedText = performance?.totalPl == null ? "Unavailable" : `$${esc(performance.totalPl)}`;
  const performanceNetAfterCostsText = performance?.netAfterCosts == null ? "Unavailable" : `$${esc(performance.netAfterCosts)}`;
  const performancePanel = performance
    ? `<details class="earnings-overlay performance-${esc(performance.tone ?? "neutral")}">
<summary aria-label="Open earnings period selector"><span>${esc(performanceLabel)} EARNINGS</span><strong>${performanceTotalText}</strong></summary>
...
<p>Realized: ${performanceRealizedText} · Unrealized: ${performanceUnrealizedText} · Combined: ${performanceCombinedText} · Net after costs: ${performanceNetAfterCostsText}</p>
...
<div><span>Last result</span><b>${esc(customerTokenLabel(postMarketLastResult?.status ?? postMarket.lastStatus ?? "waiting", "Waiting"))}</b></div>
</div>
