...
export function appendAiLogicOperatorApprovalConsumptionRecord(record,filePath=DEFAULT_PATH){
  ...
  for(const k of ["approvalRecordId","nonce","action","decisionRecordId","candidateSourceHash","candidatePath","candidateTopic","currentSourceCommit","targetSourceCommit"]) if(!present(record[k])) throw new Error(`operator_approval_consumption_${k}_required`);
  ...
  const identity={approvalRecordId:record.approvalRecordId,nonce:record.nonce,action:record.action,decisionRecordId:record.decisionRecordId,candidateSourceHash:record.candidateSourceHash,candidatePath:record.candidatePath,candidateTopic:record.candidateTopic,currentSourceCommit:record.currentSourceCommit,targetSourceCommit:record.targetSourceCommit};
