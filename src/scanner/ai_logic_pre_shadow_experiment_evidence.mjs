...
export function buildAiLogicPreShadowExperimentEvidence(input={}){
 ...
 if(!present(b.candidateId)||!present(b.candidateSourceHash)||!present(b.candidatePath)||!present(b.candidateTopic)||!present(b.replayId)||!present(b.sourceCommitBefore)||!present(b.sourceCommitAfter))reasons.push("ACCEPTANCE_IDENTITY_INCOMPLETE");
 ...
 if(b.candidateId!==orchestrator.candidateId||b.candidateSourceHash!==orchestrator.sourceHash||b.candidatePath!==orchestrator.candidatePath||b.candidateTopic!==orchestrator.candidateTopic||b.replayId!==orchestrator.safety?.replay?.replayId)reasons.push("ORCHESTRATOR_BINDING_MISMATCH");
 if(Object.prototype.hasOwnProperty.call(input,"shadowResults"))reasons.push("SHADOW_RESULTS_FORBIDDEN_PRE_SHADOW");
 const valid=reasons.length===0,binding=Object.freeze({knownGoodRecordId:present(knownGood.recordId)?knownGood.recordId:null,candidateId:present(b.candidateId)?b.candidateId:null,candidateSourceHash:present(b.candidateSourceHash)?b.candidateSourceHash:null,candidatePath:present(b.candidatePath)?b.candidatePath:null,candidateTopic:present(b.candidateTopic)?b.candidateTopic:null,replayId:present(b.replayId)?b.replayId:null,sourceCommitBefore:present(b.sourceCommitBefore)?b.sourceCommitBefore:null,sourceCommitAfter:present(b.sourceCommitAfter)?b.sourceCommitAfter:null});
