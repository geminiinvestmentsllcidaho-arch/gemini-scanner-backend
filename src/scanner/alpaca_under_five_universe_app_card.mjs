...
export function buildAlpacaUnderFiveUniverseAppCard(source = {}, options = {}) {
  ...
  const candidates = list(source.candidates).map((candidate) => ({
    ...
    decision: candidate?.resultState ?? candidate?.decision ?? "DO_NOT_ENTER",
    ...
    canonicalAuthorizationBlockers: list(candidate?.canonicalAuthorizationBlockers ?? candidate?.strategyAuthorization?.blockers),
