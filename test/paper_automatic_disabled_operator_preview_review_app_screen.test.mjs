...
import {
  ROUTE,
  buildPaperAutomaticDisabledOperatorPreviewReviewAppScreen,
  renderPaperAutomaticDisabledOperatorPreviewReviewAppScreenHtml,
} from "../src/scanner/paper_automatic_disabled_operator_preview_review_app_screen.mjs";

test("automatic operator preview review app screen remains read-only and non-executing", async () => {
  const screen =
    await buildPaperAutomaticDisabledOperatorPreviewReviewAppScreen(
      {},
      1_000_000,
    );
  const html =
    renderPaperAutomaticDisabledOperatorPreviewReviewAppScreenHtml(screen);
...
test("server and navigation register the automatic operator preview review route", () => {
  ...
  assert.match(
    server,
    /app\.get\('\/app\/paper-automatic-disabled-operator-preview-review'/,
  );
  assert.match(
    server,
    /paper_automatic_disabled_operator_preview_review_app_screen\.mjs/,
  );
  assert.match(
    navigation,
