...
test("fails closed on hash mismatch",()=>{const d=fs.mkdtempSync(path.join(os.tmpdir(),"ai-immutable-"));const f=path.join(d,"policy.mjs");fs.writeFileSync(f,"export const X=1;\n");const a=crypto.createHash("sha256").update(fs.readFileSync(f)).digest("hex");const r=verifyImmutablePolicyManifest({rootDir:d,manifest:[{path:"policy.mjs",sha256:a.replace(/^./,a[0]==="a"?"b":"a")} ]});assert.equal(r.ok,false);assert.equal(r.disposition,"REJECT");assert.equal(r.results[0].status,"IMMUTABLE_FILE_MISMATCH")});
test("fails closed when immutable file missing",()=>{const d=fs.mkdtempSync(path.join(os.tmpdir(),"ai-immutable-missing-"));const r=verifyImmutablePolicyManifest({rootDir:d,manifest:[{path:"missing.mjs",sha256:"0".repeat(64)}]});assert.equal(r.ok,false);assert.equal(r.results[0].status,"IMMUTABLE_FILE_MISSING")});
