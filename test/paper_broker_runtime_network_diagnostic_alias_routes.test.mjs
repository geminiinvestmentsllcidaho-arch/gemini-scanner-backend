...
test("paper broker runtime and network diagnostic aliases are registered read-only", () => {
  ...
  assert.match(server, /buildPaperBrokerNetworkAttemptStatusAppScreen\(\{ loadReport: true \}\)/);
  ...
  const block = server.slice(runtimeStart, server.indexOf("app.get('/app/paper-broker-network-attempt-status'", runtimeStart));
