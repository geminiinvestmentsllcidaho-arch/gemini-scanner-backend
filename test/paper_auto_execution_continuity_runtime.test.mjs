...
test('strongest eligible ENTER uses production score ordering with deterministic symbol tie-break',async()=>{const d=tmp(),old=path.join(d,'old.json');terminal(old);let active=old;const r=createPaperAutoExecutionContinuityRuntime({env:{PAPER_AUTO_CONTINUITY_ENABLED:'1'},runsDir:d,getActiveLifecycleFile:()=>active,setActiveLifecycleFile:f=>{active=f},getScanSnapshot:async()=>({observedAt:'2026-08-13T01:00:00Z',candidates:[{symbol:'ZZZ',state:'ENTER',buyRecommendation:true,score:95},{symbol:'AAA',state:'ENTER',buyRecommendation:true,score:95},{symbol:'MID',state:'ENTER',buyRecommendation:true,score:90},{symbol:'BLOCK',state:'WAIT',buyRecommendation:false,score:100}]}),idFactory:()=> 'ranked-life'});const out=await r.runOnce();assert.equal(out.lastStatus,'FRESH_CANDIDATE_LIFECYCLE_CREATED');assert.equal(out.lastLifecycle.selectedSymbol,'AAA')})
