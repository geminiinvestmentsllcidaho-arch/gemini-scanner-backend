...
test('fresh EXIT invokes exact runner once', async () => {
  ...
  assert.equal(r.lastExitDecision?.strategyExit,false)
  assert.equal(r.lastExitDecision?.protectiveExit,true)
  assert.equal(r.lastExitDecision?.protectiveType,'hard_loss')
  assert.equal(r.lastExitDecision?.priority,'critical')
  assert.equal(r.lastExitDecision?.severity,'critical')
