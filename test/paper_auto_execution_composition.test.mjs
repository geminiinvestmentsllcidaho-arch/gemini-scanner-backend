...
test('disabled by default and start prohibited', async () => {
  ...
  try {
    let calls = 0
    const c = createPaperAutoExecutionComposition({ lifecycleStore: store, submitPaperOrder: async () => { calls += 1 }, env: {} })
...
test('composition no longer depends on obsolete stage proof and still fails closed without an eligible scan dependency', async () => {
  const { dir, store } = fixture()
  try {
    let calls = 0
    const c = createPaperAutoExecutionComposition({ lifecycleStore: store, submitPaperOrder: async () => { calls += 1 }, env })
    assert.equal((await c.runOnce()).lastResult.status, 'COMPOSITION_NO_SUBMISSION_ELIGIBLE')
