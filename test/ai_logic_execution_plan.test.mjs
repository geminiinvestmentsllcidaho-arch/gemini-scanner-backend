...
test("fails closed on candidate source hash drift",()=>{let f=fx();assert.equal(b({...f,operatorApproval:{...f.operatorApproval,candidateSourceHash:"d".repeat(64)}}).eligible,false);f=fx();assert.equal(b({...f,consumptionStoreRecord:{...f.consumptionStoreRecord,candidateSourceHash:"d".repeat(64)}}).eligible,false)});
