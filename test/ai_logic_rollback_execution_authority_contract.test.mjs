...
const evidence=()=>({version:"ai_logic_rollback_decision_evidence_store_v1",recordId:"rb1",candidateId:"c1",knownGoodRecordId:"k1",replayId:"r1",sourceCommitBefore:"before",sourceCommitAfter:"after",immutableManifestStatus:"IMMUTABLE_MANIFEST_VERIFIED",rollbackTargetIdentified:true,rollbackDecisionEvidenceOnly:true,localJsonlOnly:true,...locks});
const approval=()=>({explicitlyApproved:true,oneShot:true,paperOnly:true,noLiveTradingAcknowledged:true,noImmutablePolicyMutationAcknowledged:true,rollbackDecisionRecordId:"rb1",candidateId:"c1",knownGoodRecordId:"k1",replayId:"r1",sourceCommitBefore:"before",sourceCommitAfter:"after"});
...
test("fails closed on missing approval or any approval identity drift",()=>{
  ...
  for(const k of ["rollbackDecisionRecordId","candidateId","knownGoodRecordId","replayId","sourceCommitBefore","sourceCommitAfter"]){
