...
test("fails closed on missing approval or any approval identity drift",()=>{
  ...
  for(const k of ["rollbackDecisionRecordId","candidateId","knownGoodRecordId","replayId","sourceCommitBefore","sourceCommitAfter","candidateSourceHash","candidatePath","candidateTopic"]){
...
test("candidate path and topic are required bound and preserved",()=>{const r=build(base());assert.equal(r.candidatePath,"src/scanner/ai_logic_candidates/c1.mjs");assert.equal(r.candidateTopic,"classification_coverage");for(const k of ["candidatePath","candidateTopic"]){const e=evidence();delete e[k];assert.equal(build({...base(),rollbackEvidence:e}).eligible,false);assert.equal(build({...base(),operatorApproval:{...approval(),[k]:"drift"}}).eligible,false)}});
